...
// $Id: TakeHandler.java 1306 2025-04-17 19:12:49Z awolde1 $
 ...
 */
class TakeHandler implements ActionHandler {
    ...
    public String handleAction(PlayerCommand playerCommand) {
        ...
        return itemFromAlias
                .map(item -> handleAction(item, playerCommand)) // Item found, process the take action
        ...
        if (itemList.isEmpty()) {
            ...
            return itemStream
                    .map(item -> item.getItemAliases().isEmpty()
                                 ...
                                 : this.handleAction(item, playerCommand))
